...
for teams in range(0, len(club_idt)):
        ...
        for i in range(0, len(name)):
                Player_Pitcher_Stats[i] = dict.fromkeys({'G', 'Win', 'Lose', 'HLD', 'SV', 'IP', 'Batter', 'AB', 'H','HR', 'SAC', 'SF',
        ...
        for repeat in range(0, len(file_list)):
                ...
                for i in range(0, len(name)):
                    if(load_ws.cell(Pitcher_count+i,1).value != None):
                        ...
                        if(load_ws.cell(Pitcher_count+Pitcher_away+1,1).value != None):
                            Pitcher_home+=1
                ...
                #출장 게임수 넣기
                for i in range(0, Pitcher_all):
                    for j in range(0, len(name)):
                        if(load_ws.cell(Pitcher_count+i, 1).value == name[j]):
                            Player_Pitcher_Stats[j]['G'] +=1
                            break
                
                for j in range(0, Pitcher_all): #전체선수숫자조회
                    if(load_ws.cell(Pitcher_count+j, 2).value == '승'):
                        Player_Pitcher_Stats[j]['Win'] +=1
                    elif(load_ws.cell(Pitcher_count+j, 2).value == '패'):
                        Player_Pitcher_Stats[j]['Lose'] +=1
                    elif(load_ws.cell(Pitcher_count+j, 2).value == '홀'):
                        Player_Pitcher_Stats[j]['Lose'] +=1
                    elif(load_ws.cell(Pitcher_count+j, 2).value == '세'):
                        Player_Pitcher_Stats[j]['Lose'] +=1
                    elif(load_ws.cell(Pitcher_count+j, 2).value != '-'):
                        print(load_ws.cell(Pitcher_count+j, 2).value)

                for j in range(0, Pitcher_all): #전체선수숫자조회
                    if(load_ws.cell(Pitcher_count+j, 3).value != None):
                        Player_Pitcher_Stats[j]['IP'] += float(load_ws.cell(Pitcher_count+j, 3).value.split(' ')) + .replace("⅓","+0.333").replace("⅔","+0.666"))
                        Player_Pitcher_Stats[j]['Batter'] += int(load_ws.cell(Pitcher_count+j, 4).value)
                        Player_Pitcher_Stats[j]['AB'] += int(load_ws.cell(Pitcher_count+j, 5).value)
                        Player_Pitcher_Stats[j]['H'] += int(load_ws.cell(Pitcher_count+j, 6).value)
                        Player_Pitcher_Stats[j]['HR'] += int(load_ws.cell(Pitcher_count+j, 7).value)
                        Player_Pitcher_Stats[j]['SAC'] += int(load_ws.cell(Pitcher_count+j, 8).value)
                        Player_Pitcher_Stats[j]['SF'] += int(load_ws.cell(Pitcher_count+j, 9).value)
                        Player_Pitcher_Stats[j]['BB'] += int(load_ws.cell(Pitcher_count+j, 10).value)
                        Player_Pitcher_Stats[j]['HBP'] += int(load_ws.cell(Pitcher_count+j, 11).value)
                        Player_Pitcher_Stats[j]['K'] += int(load_ws.cell(Pitcher_count+j, 12).value)
                        Player_Pitcher_Stats[j]['WP'] += int(load_ws.cell(Pitcher_count+j, 13).value)
                        Player_Pitcher_Stats[j]['BK'] += int(load_ws.cell(Pitcher_count+j, 14).value)
                        Player_Pitcher_Stats[j]['R'] += int(load_ws.cell(Pitcher_count+j, 15).value)
                        Player_Pitcher_Stats[j]['ER'] += int(load_ws.cell(Pitcher_count+j, 16).value)
                        Player_Pitcher_Stats[j]['PI'] += int(load_ws.cell(Pitcher_count+j, 17).value)
