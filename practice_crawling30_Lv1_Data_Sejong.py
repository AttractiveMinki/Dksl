        ...
        for number_of_times in range(0, len(game_url)):
            ...
            for i in range(0, len(see_Pitcher)):
                if(len(see_result_p) < 2):#한 쪽 선발투수만 입력된 경우 오류 방지
                        if(Team_away == ' ' +team_name[0].text): #우리 팀이 원정팀이면
                                sequence = len(see_Pitcher) # 원정팀 등판 투수 숫자 = sequence
                        elif(Team_home == ' ' +team_name[0].text): #우리 팀이 홈팀이면
                                sequence = 0 #원정팀 등판 투수 숫자 = 0
                        break
